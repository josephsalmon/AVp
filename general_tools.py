...
def PredictionErrorEstimated(X,coefs_path,y):
    ...
    if np.ndim(coefs_path) == 1:
        ...
        n_features, n_kinks = coefs_path.shape
        Err = np.sum((np.tile(y, (n_kinks, 1)).T -
                    np.dot(X, coefs_path))**2,0)
...
def AV_p_indexes_to_consider(index_list_ordered,j):
    ...
    list_of_indexes_to_consider = [i for i,x in enumerate(index_list_ordered) if x ==to_test]
    list_of_indexes_to_consider=list(set(list_of_indexes_to_consider+range(j,length)))
    list_of_indexes_to_consider.remove(j)
    list_of_indexes_to_consider.sort(reverse=False)
    # put true to change the order of visiting the j for testing S_ij
    return list_of_indexes_to_consider
...
def ThRR_grid(beta_Ridge, n_alphas, max_nb_variables):
    ...
    alpha_th= beta_abs[sort_beta_abs[(np.linspace(0, max_nb_variables, n_alphas,endpoint= False)).astype(int)]]
    return alpha_th

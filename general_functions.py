...
from general_tools import Support, My_nonzeros, hardthresh, ThRR_grid, \
...
def prior_fun(length_i, n_features):
    """definition of prior used in Bellec 2016 """
...
class ThRR(LinearModel, RegressorMixin):
    ...
    def fit(self, X, y):
        coef = np.squeeze(RidgePath(X, y, self.alpha, np.array([self.thresh]),
...
def ThRRAVp(X, y, alpha_ridge,a_param, n_alphas,
    ...
    coef_ThRRAVp, _, idx_ThRRAVp, _ = AVp(X, y, idx_list, idx_size, a_param)
...
def LSThRR2DCV(X, y, alpha_ridge2D, n_alphas, max_nb_variables, n_jobs=1, cv =           10,tol=1e-7, fit_intercept=False):
    ...
    for i in range(n_alphas_ridge):
        beta_Ridge = ComputeRidgeCoef(X, y, alpha_ridge2D[i],
                                      fit_intercept=fit_intercept, tol=tol)
        alpha_th = ThRR_grid(beta_Ridge, n_alphas, max_nb_variables)
        param_grid.append(dict(alpha=[alpha_ridge2D[i]], thresh=alpha_th))
...
def ThRR2DAVp(X, y, alpha_ridge2D, a_param, n_alphas,
    ...
    coef_ThRR2DAVp, _, idx_ThRR2DAVp, support_th_for = AVp(X, y, idx_list,
                                                           idx_size, a_param)
...
def LassoCV_joe_thenLS(X, y, alpha_grid, n_jobs=1, cv=10, max_iter=10000,
                       tol=1e-7, fit_intercept=False, eps_machine=1e-12):
    param_grid = dict(alpha=alpha_grid)
    sr_test = Lasso(max_iter=max_iter, tol=tol, fit_intercept=fit_intercept)
    gs = GridSearchCV(sr_test, cv=cv, param_grid=param_grid, n_jobs=n_jobs)
    ...
    n_features = coef_LassoCV.shape
    indexes_to_keep = My_nonzeros(coef_LassoCV, eps_machine=eps_machine)
    if len(indexes_to_keep) == 0:
        coef = np.zeros(n_features[0],)
    else:
        coef = np.zeros(n_features[0],)
        regr.fit(X[:, indexes_to_keep], y)
        coef[indexes_to_keep] = regr.coef_
    return coef, index_LassoCV


def LassoIC_joe(X, y, alpha_grid, method_type='bic',
                max_iter=10000,
                tol=1e-7, fit_intercept=False, eps_machine=1e-12):
    _, coefs_Lasso, _ = lasso_path(X, y, alphas=alpha_grid,
                                   fit_intercept=False,

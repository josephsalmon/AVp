...
from general_functions import LSLassoCV_joe, LassoCV_joe, LassoIC_joe, LassoBIC
from general_functions import LSThRRCV, ThRRCV, RidgePath, ComputeRidgeCoef
from general_functions import LassoAVp, LassoAVp_for_display, ThRRAVp,\
    ThRRAVp_for_display
from general_functions import QAgg

from sklearn.linear_model import lasso_path, RidgeCV
...
scenario = 'ScenarioEquiCor'  # 'ScenarioMEG'#'ScenarioEquiCor'
...
coef_SqrtLasso_SZ, sig_hat_SZ = SqrtLasso_SZ(X, y, alpha_SZ, max_iter,
                                             sig_precision, Lasso_tol,
                                             coef_ini)
...
if do_Lasso is True:
    ...
    coef_LassoBIC, _ = LassoIC_joe(X, y, alpha_grid,
                                   eps_machine=1e-12,
                                   method_type='bic')
    ...
    coef_LSLassoBIC, index_LSLassoBIC = LassoBIC(X, y, alpha_grid,
                                                 max_iter, tol,
                                                 a_param_estimated)
    end_LSLassoBIC = time.time()
    timing_LSLassoBIC = end_LSLassoBIC - start_LSLassoBIC + timing_LassoPath +\
        timing_LSLassoPath
    ...
    y_QAgg = QAgg(X, y, alpha_grid, max_iter, tol, a_param_true)
    ...
    coef_LassoAVp_sighat, index_LassoAVp_sighat = LassoAVp(X, y, alpha_grid,
                                                           max_iter, tol,
                                                           a_param_estimated)
    end_LassoAVp_sighat = time.time()
    timing_LassoAVp_sighat = end_LassoAVp_sighat - start_LassoAVp_sighat + \
        timing_SqrtLasso_SZ
    ...
    if verbose_res is True:

        print '###################    SPARSITY index     #####################'
        print
        print ['True sparsity', 'CV', 'BIC', 'LSBIC', 'CV+LS', 'LSLassoCV',
               'AVp', 'AVp_sighat', 'Sca-Lasso']
        ...
        print ''
        print ['CV', 'BIC', 'LSBIC', 'LSLassoCV', 'AVp',
               'AVp_sighat', 'Sca-Lasso', 'Q-Agg']
    ...
    plt.suptitle(scenario_str + r" $(p={0}, n={1}, s={2}, \sigma={3},\rho={4}, a={5}\sigma^2, r={6})$".format(n_features, n_samples, s, sig_noise, rho, AVp_constant, n_alphas),
                 fontsize=title_fontsize, multialignment='center')

    ax1 = fig3.add_subplot(321)
    ax1.plot(xaxis_val, PredictionError(X, coefs_Lasso, beta_true), '--',
             color=current_palette[0], label="lasso")
    ax1.plot(xaxis_val, PredictionError(X, coefs_LSLasso, beta_true), ':',
    ...
    ax1.plot(index_LSLassoCV + 1, PredictionError(X, coef_LSLassoCV,
             beta_true), color='blue', marker='^', linestyle='None',
             markersize=20, label="lslassoCV")
    ax1.plot(index_LassoAVp + 1, PredictionError(X, coef_LassoAVp, beta_true),
             'rD', markersize=9, label=r"lassoAV${}_{\mbox{{p}}}$")
    ax1.plot(index_LassoAVp_sighat + 1, PredictionError(X,
                                                        coef_LassoAVp_sighat,
                                                        beta_true),
             '*', color=current_palette[2], markersize=24,
             label=r"lassoAV${}_{\mbox{{p}}}$ with $\hat\sigma$")
    ...
    ax2.plot(xaxis_val, EstimationError_2(coefs_Lasso, beta_true), '--',
             color=current_palette[0], label="lasso")
    ...
    ax2.plot(index_LSLassoCV + 1, EstimationError_2(coef_LSLassoCV, beta_true),
             color='blue', marker='^', linestyle='None', markersize=20,
             label="lslassoVB")
    ax2.plot(index_LassoAVp + 1, EstimationError_2(coef_LassoAVp, beta_true),
             'rD', markersize=9, label=r"lassoAV${}_{\mbox{{p}}}$")
    ax2.plot(index_LassoAVp_sighat + 1, EstimationError_2(coef_LassoAVp_sighat,
             beta_true),
             '*', color=current_palette[2], markersize=24,
             label=r"lassoAV${}_{\mbox{{p}}}$ with $\hat\sigma$")
    ...
    ax3.plot(xaxis_val, EstimationError(coefs_Lasso, beta_true), '--',
             color=current_palette[0], label="lasso")
    ax3.plot(xaxis_val, EstimationError(coefs_LSLasso, beta_true), ':',
             color=current_palette[1], label="lasso")
    ...
    ax3.plot(index_LassoAVp + 1, EstimationError(coef_LassoAVp, beta_true),
             'rD', markersize=9, label=r"lassoAV${}_{\mbox{{p}}}$")
    ax3.plot(index_LassoAVp_sighat + 1, EstimationError(coef_LassoAVp_sighat,
             beta_true),
             '*', color=current_palette[2], markersize=24,
             label=r"lassoAV${}_{\mbox{{p}}}$ with $\hat\sigma$")
    plt.xlabel(xlabels, fontsize=label_fontsize)
    ...
    ax4.plot(xaxis_val, FalsePositive(coefs_Lasso, beta_true), '--',
             color=current_palette[0], label="lasso")
    ax4.plot(xaxis_val, FalsePositive(coefs_LSLasso, beta_true), ':',
             color=current_palette[1], label="lslasso")
    ...
    ax4.plot(index_LassoAVp + 1, FalsePositive(coef_LassoAVp, beta_true),
             'rD', markersize=9, label=r"lassoAV${}_{\mbox{{p}}}$")
    ax4.plot(index_LassoAVp_sighat + 1, FalsePositive(coef_LassoAVp_sighat,
             beta_true),
             '*', color=current_palette[2], markersize=24,
             label=r"lassoAV${}_{\mbox{{p}}}$ with $\hat\sigma$")
    plt.xlabel(xlabels, fontsize=label_fontsize)
    ...
    ax5.plot(xaxis_val, FalseNegative(coefs_Lasso, beta_true), '--',
             color=current_palette[0], label="lasso")
    ax5.plot(xaxis_val, FalseNegative(coefs_LSLasso, beta_true), ':',
             color=current_palette[1], label="lslasso")
    ...
    ax5.plot(index_LassoAVp + 1, FalseNegative(coef_LassoAVp, beta_true),
             'rD', markersize=9, label=r"lassoAV${}_{\mbox{{p}}}$")
    ax5.plot(index_LassoAVp_sighat + 1, FalseNegative(coef_LassoAVp_sighat,
             beta_true),
             '*', color=current_palette[2], markersize=24,
             label=r"lassoAV${}_{\mbox{{p}}}$ with $\hat\sigma$")
    plt.xlabel(xlabels, fontsize=label_fontsize)
    ...
    ax6.plot(coef_LassoAVp_sighat, 'r',
             label=r"lassoAV${}_{\mbox{{p}}}$ with $\hat\sigma$")
...
if do_ThRR is True:
    ...
    RidgeCV_pred = RidgeCV(fit_intercept=False,
                           alphas=[0.01, 0.1, 1.0, 10.0, 100, 1000, 10000])
    ...
    coef_ThRRCV, index_ThRRCV = ThRRCV(X, y, alpha_ridge, n_alphas,
                                       max_nb_variables, n_jobs=1,
                                       cv=cv_fold)
    ...
    ###########################################################################
    start_RidgeDispaly = time.time()
    matrix_for = ThRRAVp_for_display(X, y, alpha_ridge, a_param_estimated,
                                     n_alphas, max_nb_variables, tol=tol,
                                     fit_intercept=fit_intercept)
    end_RidgeDispaly = time.time()
    timing_ThRRDisplay = end_RidgeDispaly - start_RidgeDispaly
    ...
    coef_ThRRAVp_sighat, index_ThRRAVp_sighat = ThRRAVp(X, y, alpha_ridge,
                                                        ...
                                                        fit_intercept=fit_intercept)
    end_ThRRAVp_partial = time.time()
    timing_ThRRAVp_sighat = end_ThRRAVp_partial - start_ThRRAVp_partial + \
        timing_SqrtLasso_SZ
    ...
    ax1.plot(xaxis_val, PredictionError(X, coefs_ThRR, beta_true), '--',
             color=current_palette[0],
             label="thrr")
    ax1.plot(xaxis_val, PredictionError(X, coefs_ThRR_LS, beta_true), ':',
    ...
    ax1.plot(index_ThRRAVp_sighat + 1, PredictionError(X, coef_LassoAVp_sighat,
             beta_true),
             '*', color=current_palette[2], markersize=24,
             label=r"$\mbox{thrrAV}_{\mbox{{p}}}$")
    ax1.plot(index_ThRRAVp + 1, PredictionError(X, coef_ThRRAVp, beta_true),
             'rD', markersize=9,
             label=r"$\mbox{thrrAV}_{\mbox{{p}}}$ with $\hat\sigma$")
    ax1.set_yscale('symlog')
    ...
    ax2.plot(xaxis_val, EstimationError_2(coefs_ThRR, beta_true), '--',
             color=current_palette[0],
             label="thrr")
    ax2.plot(xaxis_val, EstimationError_2(coefs_ThRR_LS, beta_true), ':',
             color=current_palette[1],
             label="lsthrr")
    ...
    ax2.plot(index_LSThRRCV + 1, EstimationError_2(coef_LSThRRCV, beta_true),
             'o', color=current_palette[1], markersize=20, label="lsthrrCV")
    ax2.plot(index_ThRRAVp_sighat + 1, EstimationError_2(coef_LassoAVp_sighat,
             beta_true),
             '*', color=current_palette[2], markersize=24,
             label=r"$\mbox{thrrAV}_{\mbox{{p}}}$")
    ...
    ax3.plot(xaxis_val, EstimationError(coefs_ThRR, beta_true), '--',
             color=current_palette[0], label="thrr")
    ax3.plot(xaxis_val, EstimationError(coefs_ThRR_LS, beta_true), ':',
             color=current_palette[1],
             label="lsthrr")
    ...
    ax3.plot(index_LSThRRCV + 1, EstimationError(coef_LSThRRCV, beta_true),
             'o', color=current_palette[1], markersize=20, label="lsthrrCV")
    ax3.plot(index_ThRRAVp_sighat + 1, EstimationError(coef_ThRRAVp_sighat,
             beta_true),
             '*', color=current_palette[2], markersize=24,
             label=r"$\mbox{thrrAV}_{\mbox{{p}}}$")
    ...
    ax4.plot(xaxis_val, FalsePositive(coefs_ThRR, beta_true), '--',
             color=current_palette[0], label="thrr")
    ...
    ax4.plot(index_ThRRAVp_sighat + 1, FalsePositive(coef_ThRRAVp_sighat,
             beta_true), '*', color=current_palette[2], markersize=24,
             label=r"$\mbox{thrrAV}_{\mbox{{p}}}$")
    ...
    ax5.plot(xaxis_val, FalseNegative(coefs_ThRR, beta_true), '--',
             color=current_palette[0], label="thrr")
    ax5.plot(xaxis_val, FalseNegative(coefs_ThRR_LS, beta_true), ':',
             color=current_palette[1],
             label="lsthrr")
    ...
    ax5.plot(index_LSThRRCV + 1, FalseNegative(coef_LSThRRCV, beta_true),
             'o', color=current_palette[1], markersize=20, label="lsthrrCV")
    ax5.plot(index_ThRRAVp_sighat + 1, FalseNegative(coef_ThRRAVp_sighat,
             beta_true),
             '*', color=current_palette[2], markersize=24,
             label=r"$\mbox{thrrAV}_{\mbox{{p}}}$")
    ...
    ax6.plot(coef_ThRRAVp_sighat, color=current_palette[2],
             label=r"$\mbox{thrrAV}_{\mbox{{p}}}$ with $\hat\sigma$")
    plt.xlabel('Coefficients', fontsize=label_fontsize)
    plt.title('Recovered signal', fontsize=subtitle_fontsize)
    plt.legend(prop={'size': legend_fontsize})
    plt.subplots_adjust(hspace=0.2, wspace=0.2, bottom=.1)
    plt.show()
    ...
    if verbose_res is True:
        ...
        print '####################  SPARSITY index  #########################'
        print ''
        print ['True Sparsity', 'CV', 'ThRRCV+LS', 'LSThRRCV', 'ThRRAVp',
               'ThRRAVp_sighat']
        ...
        print '###############################################################'
        print ''
        print ['RidgeCV', 'ThRRCV', 'ThRRCV+LS', 'LSThRRCV', 'ThRRAVp',
               'ThRRAVp_sighat']
...
##############################################################################
if do_Lasso is True & do_ThRR is True:
